  ...
  return <div className="flex min-h-screen flex-col bg-background">
    ...
    <main className="flex-1">
      <section className="landing-hero mx-auto grid max-w-6xl items-center gap-12 px-6 py-16 md:grid-cols-2 md:py-24">
        <div><div className="flex items-center gap-3"><Image src="/icon.svg" width={32} height={32} alt=""/><span className="font-mono text-[10px] uppercase tracking-[.2em] text-accent">{t("hero.eyebrow")}</span></div>
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Link href="/workspace" className="rounded-md bg-zinc-100 px-6 py-3 text-sm font-medium text-zinc-900 hover:bg-white">{t("hero.cta")}</Link><Link href="/dashboard" className="rounded-md border border-zinc-700 px-6 py-3 text-sm text-zinc-400 hover:border-zinc-500 hover:text-zinc-200">{t("hero.dashboard")}</Link></div>
      ...
      <section id="features" className="border-t border-zinc-800 px-6 py-20"><div className="mx-auto max-w-5xl"><p className="font-mono text-[10px] uppercase tracking-[.2em] text-accent">{t("features.eyebrow")}</p><h2 className="mt-4 text-3xl font-semibold tracking-tight">{t("features.heading")}</h2><div className="mt-10 grid gap-6 sm:grid-cols-2">{features.map(({title,body}, index) => { const Icon = icons[index]; return <div key={title} className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6"><Icon size={20} className="mb-4 text-zinc-400"/><h3 className="text-base font-medium">{title}</h3><p className="mt-3 text-sm leading-relaxed text-zinc-400">{body}</p></div>; })}</div></div></section>
      <section className="border-t border-zinc-800 px-6 py-20"><div className="mx-auto max-w-5xl"><h2 className="text-3xl font-semibold tracking-tight">{t("styles.heading")}</h2><p className="mt-4 text-zinc-400">{t("styles.body")}</p><div className="mt-10 grid grid-cols-2 gap-4 lg:grid-cols-4">{styleIds.map((id) => <Link href="/workspace" key={id} className="group"><div className={`landing-style theme-${id}`}><span>{theme(`${id}.label`)}</span><div/><div/><div/><b>✦</b></div><h3 className="mt-4 text-sm text-zinc-300 group-hover:text-white">{theme(`${id}.name`)}</h3></Link>)}</div></div></section>
      ...
      <section className="border-t border-zinc-800 px-6 py-20 text-center"><h2 className="text-3xl font-semibold tracking-tight">{t("closing.heading")}</h2><Link href="/workspace" className="mt-7 inline-block rounded-md bg-zinc-100 px-6 py-3 text-sm font-medium text-zinc-900 hover:bg-white">{t("closing.cta")}</Link></section>
